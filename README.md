...
**כבר ב-1966 אנשים ישבו מול מסוף ושוחחו עם תוכנה - וסיפרו לה דברים שלא
סיפרו לאיש.**

בינה מלאכותית? לא. כמה עשרות כללים שאדם כתב ביד.

זו התוכנית שוייצנבאום כתב במכון הטכנולוגי של מסצ'וסטס בין 1964 ל-1966,
בנויה מחדש כתוכנה לווינדוס - ובעברית.

קובץ הפעלה אחד, כ-5.1 מגה־בייט, בלי התקנה ובלי צורך בחיבור לאינטרנט. בפנים:
...
וחמשת האחרים כבר אינם אלייזה. הם עושים בדיוק את אותו הדבר — מחפשים מילת
...
`(SCENE ...)`. ההוראות שתסריט יכול לשאת, כולן אופציונליות:

```
(NAME שם על הכרטיס)
(ABOUT משפט אחד מתחתיו)
(SCENE BEIS)             החדר. שמונה: MUSEUM · TELETYPE · BEIS ·
                         MUSSAR · TAPES · PARLOUR · COURT · CORNER
(FAMILY MODEL)           על איזה מדף במסך הפתיחה
(DIRECTION RTL)          כיוון הכתיבה
(GREETINGS (...) (...))  שורות פתיחה, אחת מהן נבחרת
(NOMATCH (...) (...))    הודעות המילוט, במקום אלה של התסריט
(SILENCE (...) (...))    מה לומר למי ששותק
(DELIMITERS . ? !)       מה חותך משפט
(FEMININE-MARKERS ...)   מילים שמסגירות את מין הדובר
(MASCULINE-MARKERS ...)
```
...
ובדיקת העדכונים כבויה כברירת מחדל. כל עוד היא כבויה התוכנה אינה פונה
לרשת בשום שלב, וזו הנקודה: היא נועדה להישאר על מקל ולעבוד במחשב מנותק.
...
בתעודה מסחרית: לוחצים **מידע נוסף** (*More info*) ואז **הפעל בכל זאת**
(*Run anyway*).

בהגדרות יש מתג לבדיקת עדכונים, והוא כבוי. כשמדליקים אותו היא שואלת את
גיטהאב מה שם המהדורה האחרונה, ואם יש חדשה — מורידה ומחליפה את עצמה
בלחיצה אחת, בלי להתקין ובלי לגעת בהגדרות שנשמרו.
...
במעטפת: הספרות `1` עד `8` נכנסות לתסריט, `Ctrl+1` עד `Ctrl+5` עוברות בין
...
### להמשיך שיחה — ולשלוח אותה למישהו

כל שיחה נשמרת מעצמה כקובץ טקסט. אפשר לפתוח אותה שוב ולהמשיך לדבר מאותה
נקודה: בעמוד **תמלילים**, פותחים שיחה וקוראים, ולוחצים **המשך השיחה**.

ואפשר לשלוח אותה. מי שמקבל את הקובץ פותח אותו — מכפתור הפתיחה שבאותו עמוד,
בגרירה של הקובץ אל החלון, או `ELIZA.exe שיחה.txt` — וממשיך מאיפה שהפסקתם.

**וזה עובד גם על שיחה שכתבתם בעצמכם.** המנוע אינו קורא את השיחה: כל תשובה
מחושבת מהמשפט האחרון ומארבעה מונים, והמונים תלויים **רק במה שאתם הקלדתם**.
לכן אפשר לחתוך שיחה באמצע, לשנות את מה שהיא ענתה, או לכתוב אחת מאפס
בפנקס רשימות — והיא תמשיך משם כאילו כל זה קרה. הפורמט הוא שורה אחת:

```
· שורה שמתחילה בנקודה היא מה שאתם אמרתם
וכל שורה אחרת היא מה שהיא אמרה
```

מה שעל המסך הוא הקובץ, מילה במילה. מה שהיא יודעת בא מהשורות שלכם.
...
המנוע, המעטפת, התסריט העברי וששת השי-בוטים נכתבו לפרויקט הזה ומשוחררים
ברישיון MIT — ראו [LICENSE](LICENSE).

`scripts/doctor.txt` אינו שלנו: הוא תעתיק של נספח התסריט מתוך המאמר של
וייצנבאום משנת 1966, ומובא כאן בשלמותו לצורכי לימוד עם הייחוס שהתעתיק
נושא. זכויות המאמר שמורות ל-ACM.

---

## מקורות וזכויות

- **המאמר שהתסריט של 1966 בא ממנו**: וייצנבאום,
